/*
 * drivers/cpufreq/cpufreq_lulzactivev2.c
 *
 ...
 *
 * Driver values in /sys/devices/system/cpu/cpufreq/lulzactivev2
 * 
 */
...
//a hack to make comparisons easier while having different structs in pegasusq and lulzactivev2
...
static struct dbs_tuners {
    ...
    /* hotplug tuners - from lulzactivev2 */
...
int cpufreq_lulzactivev2_cpu_lock(int num_core)
{
	int prev_lock;
...
int cpufreq_lulzactivev2_cpu_unlock(int num_core)
{
...
void cpufreq_lulzactivev2_min_cpu_lock(unsigned int num_core)
{
	int online, flag;
	struct cpufreq_lulzactive_cpuinfo *dbs_info;
...
void cpufreq_lulzactivev2_min_cpu_unlock(void)
{
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret != 1)
		return -EINVAL;
	if (input == 0)
		cpufreq_lulzactivev2_min_cpu_unlock();
	else
		cpufreq_lulzactivev2_min_cpu_lock(input);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_lulzactivev2_cpu_unlock(prev_lock);
	...
	ret = cpufreq_lulzactivev2_cpu_lock(input);
	if (ret) {
...
void start_lulzactivev2(void);
void stop_lulzactivev2(void);
...
static int __init cpufreq_lulzactive_init(void)
{
	...
	dvfs_workqueue = create_workqueue("klulzactivev2");
	if (!dvfs_workqueue) {
	...
	down_wq = alloc_workqueue("klulzactivev2_down", 0, 1);
...
MODULE_DESCRIPTION("'lulzactivev2' - improved lulzactive governor with hotplug logic");
MODULE_LICENSE("GPL");
